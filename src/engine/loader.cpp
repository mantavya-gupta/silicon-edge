...
static Q4Matrix read_q4(std::ifstream& f) {
    Q4Matrix m;
    f.read((char*)&m.rows, 4);
    f.read((char*)&m.cols, 4);
    m.scales.resize(m.rows);
    f.read((char*)m.scales.data(), m.rows * 4);
    m.data.resize(m.rows * (m.cols / 2));
...
LlamaModel LlamaModel::load(const std::string& path) {
    ...
    for (auto& l : m.layers) {
        ...
        l.v_proj   = read_q4(f);
        l.o_proj   = read_q4(f);
        l.gate_proj= read_q4(f);
        l.up_proj  = read_q4(f);
        l.down_proj= read_q4(f);
    }
