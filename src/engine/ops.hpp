...
inline void softmax(float* x, int n) {
    float mx = *std::max_element(x,x+n), sum=0;
...
inline void rope(float* x, int head_dim, int pos, float theta=500000.0f) {
    for (int i=0;i<head_dim;i+=2){
        float freq=1.0f/powf(theta,(float)i/head_dim);
        float c=cosf(pos*freq),s=sinf(pos*freq);
        float x0=x[i],x1=x[i+1];
        x[i]=x0*c-x1*s; x[i+1]=x0*s+x1*c;
    }
}
inline void add(float* a, const float* b, int n){for(int i=0;i<n;i++)a[i]+=b[i];}
inline void elemul(float* a, const float* b, float* out, int n){for(int i=0;i<n;i++)out[i]=a[i]*b[i];}
